...
print(np.max(1./a1),np.min(1./a1),np.max(1./a1)/np.min(1./a1))



#plot sky image after dark and flat
#image in arcsinh scale
plt.figure('clean')
plt.imshow(new1)
plt.axis('off')  # clear x- and y-axes
plt.title('image after dark and flat: arcsinh')
...
n, bins, patches = plt.hist(np.log(new), num_bins, label='data')
